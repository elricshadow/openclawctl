...
> **⚠️ 注意：此版本由 Zevon 维护，集成了私有化适配与多 Profile 兼容功能。**

---

## 🚀 Zevon 版新增/修改点 (Change Log)
1.  **私有化闭环**：所有内部脚本拉取链接已重定向至 `elricshadow/openclawctl`，确保完全在你的私有 GitHub 环境中运行。
2.  **环境隔离适配**：优化了对 OpenClaw 多 Profile（如 `--profile test_lobster`）的支持逻辑，避免主 Agent 配置冲突。
3.  **安全路径加固**：修正了原始脚本中硬编码的安装路径，提升了在不同服务器环境下的兼容性。
